...
case "$CMD" in
    ...
    create)
        ...
        if [ -n "$BODY_FILE" ]; then
            TMP_JSON=$(mktemp)
            python3 - "$TITLE" "$BODY_FILE" "$TMP_JSON" <<'PYEOF'
import json, sys
title, body_file, out_file = sys.argv[1:4]
...
PYEOF
            ...
            "$API" -X POST -d "{\"title\":\"$TITLE\",\"body\":\"$BODY\"}" "repos/$REPO/issues"
        else
            "$API" -X POST -d "{\"title\":\"$TITLE\"}" "repos/$REPO/issues"
        fi
        ;;
    close)
        NUM="${3:-}"
